...
async def voice_webhook(request: Request):
    ...
    xml_response = f"""<?xml version="1.0" encoding="UTF-8"?>
        <Response>
          <Connect>
            <ConversationRelay url='{websocket_url}' welcomeGreeting="{AGENT_GREETING}"/>
          </Connect>
        </Response>
        """

    logger.info(f"XML response:\n{xml_response}")
...
async def websocket_endpoint(websocket: WebSocket, call_sid: str):
    ...
    """
    print(f"[DEBUG] WebSocket connection attempt for call_sid={call_sid}", flush=True)
    logger.info(f"WebSocket connection attempt for call_sid={call_sid}")

    await websocket.accept()
    print(f"[DEBUG] WebSocket connection accepted for call_sid={call_sid}", flush=True)
    logger.info(f"WebSocket connection accepted for call_sid={call_sid}")

    # Get or create session
    print(f"[DEBUG] Checking session for call_sid={call_sid}", flush=True)
